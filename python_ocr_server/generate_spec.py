...
def get_onnxruntime_binaries():
    ...
    if system == "Darwin":  # macOS
        ...
        if dylib_files:
            binaries.append((dylib_files[0].as_posix(), 'onnxruntime/capi'))

        # pybind11 state (usually .so even on macOS)
        pybind_file = onnx_capi / "onnxruntime_pybind11_state.so"
        if pybind_file.exists():
            binaries.append((pybind_file.as_posix(), 'onnxruntime/capi'))

    elif system == "Linux":
        # Look for .so files
        so_files = list(onnx_capi.glob("libonnxruntime.so.*"))
        if so_files:
            binaries.append((so_files[0].as_posix(), 'onnxruntime/capi'))
        ...
        if dll_files:
            for dll in dll_files:
                # Use forward slashes for Windows paths in spec file
                binaries.append((dll.as_posix(), 'onnxruntime/capi'))

        # pybind11 state (usually .pyd on Windows)
        pyd_file = onnx_capi / "onnxruntime_pybind11_state.pyd"
        if pyd_file.exists():
            binaries.append((pyd_file.as_posix(), 'onnxruntime/capi'))

        # Also check for .so (sometimes present)
        so_file = onnx_capi / "onnxruntime_pybind11_state.so"
        if so_file.exists():
            binaries.append((so_file.as_posix(), 'onnxruntime/capi'))
...
def generate_spec():
    ...
    # Convert site_packages path to POSIX style (forward slashes) for cross-platform compatibility
    rapidocr_path = (site_packages / "rapidocr").as_posix()
